# dosssman: Environments allowing the agent to roam freely around the map, which can be usefull for diversity oriented
# experiments such as "Diversity is All You Need"
...
class WalkerFreeBase(MJCFBasedRobot):
  ...
  def __init__(self, fn, robot_name, action_dim, obs_dim, power):
    ...
    self.body_xyz = [0, 0, 0]
    # dosssman: To measure the distance traveled between 2 simulation steps, we need to store the old ones
    # to compute the difference
    self.old_body_xyz = [0, 0, 0]
  ...
  def calc_state(self):
    ...
    self.old_body_xyz = copy(self.body_xyz)
...
class HopperFree(WalkerFreeBase):
  foot_list = ["foot"]

  def __init__(self):
    WalkerFreeBase.__init__(self, "hopper.xml", "torso", action_dim=3, obs_dim=15, power=0.75)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.8 and abs(pitch) < 1.0 else -1


class Walker2DFree(WalkerFreeBase):
  foot_list = ["foot", "foot_left"]

  def __init__(self):
    WalkerFreeBase.__init__(self, "walker2d.xml", "torso", action_dim=6, obs_dim=22, power=0.40)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.8 and abs(pitch) < 1.0 else -1

  def robot_specific_reset(self, bullet_client):
    WalkerFreeBase.robot_specific_reset(self, bullet_client)
    for n in ["foot_joint", "foot_left_joint"]:
      self.jdict[n].power_coef = 30.0


class HalfCheetahFree(WalkerFreeBase):
  foot_list = ["ffoot", "fshin", "fthigh", "bfoot", "bshin",
               "bthigh"]  # track these contacts with ground

  def __init__(self):
    WalkerFreeBase.__init__(self, "half_cheetah.xml", "torso", action_dim=6, obs_dim=26, power=0.90)
...
class AntFree(WalkerFreeBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerFreeBase.__init__(self, "ant.xml", "torso", action_dim=8, obs_dim=28, power=2.5)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.26 else -1  # 0.25 is central sphere rad, die if it scrapes the ground


class HumanoidFree(WalkerFreeBase):
  self_collision = True
  foot_list = ["right_foot", "left_foot"]  # "left_hand", "right_hand"

  def __init__(self):
    WalkerFreeBase.__init__(self,
